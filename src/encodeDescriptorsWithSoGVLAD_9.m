...
%-------------------------------------------------------------------------------------------
 z1 = vl_vlad(cat(1,descrs,layout), cat(1,encoder.words,encoder.catwords), assign, 'Unnormalized');%'SquareRoot', 'NormalizeComponents');%, ...
 z1 = sqrt(z1);
 %                 'SquareRoot', ...
 %                 'NormalizeComponents'
 ...
 %z = cat(1,z1,z2);
 % inter zscore
 z1 = reshape(z1, 130, size(encoder.words,2));
 z1 = zscore(z1');
 %intra L1/L2 norm
 %for k=1:size(z1,1)
 %    vector = z1(k,:);
